...
/*
 * 1280x800 at deviceScaleFactor 1: the GIF is resized to 1000px wide for the README,
 * and recording at 2x then downscaling made the text mushy rather than crisper.
 */
...
try {
  ...
  await page.waitForTimeout(2500);
  await clickAny(
    ['button:has-text("Run preview")', 'button:has-text("Run quick preview")', 'button:has-text("Compare")'],
    'run preview',
  );
...
/*
 * A single-pass GIF of a mostly-white UI dithers into visible noise and lands around
 * 40 MB. Generating a per-clip palette first keeps text edges clean at ~1/4 the size.
 * 10 fps is the floor where the scroll still reads as motion.
 */
const palette = join(WORK, 'palette.png');
run(['-i', mp4, '-vf', 'fps=10,scale=1000:-1:flags=lanczos,palettegen=stats_mode=diff', palette]);
run([
  '-i', mp4, '-i', palette,
