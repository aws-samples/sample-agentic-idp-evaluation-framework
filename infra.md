...
| File | Manages |
|---|---|
| `main.tf` | Provider config (aws, random), S3 backend (bucket passed via `-backend-config`), us-east-1 alias for ACM |
...
- **CORS**: localhost:5173 (개발)과 idp.example.com (프로덕션)만 허용
- **퍼블릭 차단**: 전면 활성화
...
State에는 `random_password.cloudfront_secret`이 포함되므로, **state 파일 접근 권한을 제한**해야 합니다. DynamoDB lock은 기본 미설정이므로 팀 환경에서는 별도 구성하세요.

## Key Outputs

```bash
terraform output site_url                 # https://idp.example.com
...
terraform output s3_bucket_name           # <project_name>-uploads-<environment>
terraform output static_assets_bucket     # <project_name>-static-<environment>
```
