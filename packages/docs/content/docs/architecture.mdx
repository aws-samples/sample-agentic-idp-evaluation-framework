...
|---|---|---|
| **Edge** | CloudFront distribution `<your-distribution-id>` | HTTPS termination, SPA caching, path-based origin routing, 60 s read / 30 s keepalive to App Runner. |
...
| `ADMIN_USERS` | `alice,bob` | Comma-separated aliases with admin access. |
