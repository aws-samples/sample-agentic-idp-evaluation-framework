...
It is aimed at **AWS Solutions Architects and customers** who need to answer one question: *"which AWS document processing approach best fits my use case, and what does it cost?"*

## What you get

Instead of running a dozen proofs-of-concept by hand, you get:

- A single upload that is evaluated against up to **15 processing methods** (Bedrock Data Automation, Claude, Nova, and Amazon Textract in hybrid configurations).
...
- It is **not** a production inference service. It is an evaluation harness.
- It is **not** a blueprint editor for BDA custom blueprints (that work happens in the Bedrock console).
- It does not claim the confidence scores are ground truth — models grade their own homework. Treat confidence as a *tiebreaker* after cost and latency, not as absolute accuracy.
...
If you skip the platform and build the adapters yourself, you will end up writing roughly the same code. ONE IDP's value is that it also *benchmarks your document against all of them first* so you know which to actually ship.

## Next

- [**Quickstart**](/docs/quickstart) — upload a document and run the benchmark.
- [**The 5-step workflow**](/docs/workflow) — what each step does.
- [**Methods**](/docs/methods) — the 15 processing methods and when to pick each.
