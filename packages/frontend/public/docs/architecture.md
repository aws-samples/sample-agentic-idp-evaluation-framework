...
├── reference/            # Third-party reference projects (git submodules, not published)
└── test-samples/         # Sample documents for local evaluation
```

Build order: `shared → backend + frontend` (the latter two are parallel). The `@idp/shared` package provides the single source of truth for method IDs, pricing, capabilities, and every shared DTO.
                            ...
                            └──── /api/*  ──────► App Runner  one-idp-backend-dev
                                             ...
                                             Bedrock AgentCore Runtime
                                               one_idp_dev-2iwFLG8S4q
                                                     │
                                                     ▼
                                             Strands agent (container on
                                             multi-arch amd64+arm64 ECR image)

S3  one-idp-uploads-dev   ◄── user uploads (NFC-normalized, sanitized keys)
S3  sanghwa-oregon        ◄── local dev bucket
...
| **SPA** | S3 `one-idp-static-dev` | Vite-built React bundle. Cache-Control on static assets. |
| **API** | App Runner `one-idp-backend-dev` (arn:aws:apprunner:us-west-2:123456789012:service/one-idp-backend-dev/…) | Express server, pluggable auth, adapters. |
...
```
Browser  ──HTTPS──►  CloudFront  ──path split──►  S3 (static)  or  App Runner
                                                                     │
