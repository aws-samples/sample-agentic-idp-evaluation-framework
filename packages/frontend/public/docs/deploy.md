...
auth_provider          = "cognito"                # none | midway | cognito
...
aws s3 sync packages/frontend/dist s3://<your-static-bucket>/ --delete
...
After `cdk deploy` the ECR repo exists but is empty — push the first backend image via CodeBuild (same as Terraform path), then trigger App Runner deployment.
...
- [ ] Admin endpoint `GET /api/admin/stats` returns `403` when unauth and `200` when you're in `ADMIN_USERS`.
