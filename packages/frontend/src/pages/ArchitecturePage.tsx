...
/** Build method→capabilities map from pipeline results (actual) or static fallback */
function buildMethodMap(capabilities: Capability[], processingResults: ProcessorResult[]): Map<string, string[]> {
  const methodMap = new Map<string, string[]>();

  if (processingResults.length > 0) {
    // Use actual pipeline execution results
  ...
  // Fallback: if no results or some capabilities unmapped, fill from static
...
function generatePythonCode(capabilities: Capability[], processingResults: ProcessorResult[]): string {
  const methodMap = buildMethodMap(capabilities, processingResults);
...
function generateTypeScriptCode(capabilities: Capability[], processingResults: ProcessorResult[]): string {
  const methodMap = buildMethodMap(capabilities, processingResults);
...
export default function ArchitecturePage({
  ...
  const pythonCode = useMemo(() => generatePythonCode(capabilities, processingResults), [capabilities, processingResults]);
  const tsCode = useMemo(() => generateTypeScriptCode(capabilities, processingResults), [capabilities, processingResults]);
