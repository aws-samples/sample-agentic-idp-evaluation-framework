...
function generatePythonCode(capabilities: Capability[]): string {
  const methodMap = new Map<string, string[]>();
  for (const cap of capabilities) {
    const methods = getBestMethodsForCapability(cap);
    const best = methods[0];
    const info = METHOD_INFO[best as ProcessingMethod];
...
function generateTypeScriptCode(capabilities: Capability[]): string {
  const methodMap = new Map<string, string[]>();
  for (const cap of capabilities) {
    const methods = getBestMethodsForCapability(cap);
    const best = methods[0];
...
export default function ArchitecturePage({
  ...
  const methodSummary = useMemo(() => {
    ...
    for (const cap of capabilities) {
      ...
      const info = METHOD_INFO[best as ProcessingMethod];
      if (!methodMap.has(best)) methodMap.set(best, []);
      methodMap.get(best)!.push(CAPABILITY_INFO[cap]?.name ?? cap);
    }
